...
def deploy_online(env, controller, horizon):
    ...
    for h in range(horizon):
        ...
        if controller.name == "Transformer":
            states_lnr, actions_lnr, next_states_lnr, rewards_lnr, probs_lnr = (
                env.deploy(controller, return_probs=True)
            )
...
def online(eval_trajs, model, n_eval, horizon, var, bandit_type, mytest=False):
    ...
    for i_eval in range(n_eval):
        ...
        controller = BanditTransformerController(model, sample=True)
        cum_means = deploy_online(env, controller, horizon)
...
def offline_graph(eval_trajs, model, n_eval, horizon, var, bandit_type):
    ...
    for key in means.keys():
        if not key == "opt":
            regrets = [
                all_means[i]["opt"] - all_means[i][key] for i in range(len(horizons))
            ]
            plt.plot(horizons, regrets, label=key)
            plt.fill_between(
                horizons, regrets - sems[key], regrets + sems[key], alpha=0.2
            )
