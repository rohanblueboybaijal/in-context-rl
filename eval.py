...
if __name__ == "__main__":
    ...
    if envname == "bandit":
        ...
        eval_trajs = [
            bandit_env.sample(dim, act_num, horizon, var, type="uniform")
            for _ in range(n_eval)
        ]
    elif envname == "bandit_bernoulli":
        eval_trajs = [
            bandit_env.sample(dim, act_num, horizon, var, type="bernoulli")
            for _ in range(n_eval)
        ]
    else:
        raise NotImplementedError
    ...
    if envname == "bandit" or envname == "bandit_bernoulli":
        ...
        all_means, all_means_diff, means, sems = eval_bandit.online(
            eval_trajs, model, **config
        )
