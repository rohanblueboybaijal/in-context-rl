    ...
    def act(self, x):
        ...
        if self.env.type == "bernoulli":
            if len(self.batch["context_rewards"][0]) < 1:
                ...
                index = self.compare_actions_bernoulli(
                    np.zeros(self.env.act_num), np.zeros(self.env.act_num)
                )
                hot_vector[index] = 1
                self.t += 1
            else:
        ...
        if len(self.batch["context_rewards"][0]) < 1:
            ...
            index = self.compare_actions(
                self.theta, self.variance * np.linalg.inv(self.cov)
            )
            hot_vector[index] = 1
            self.t += 1
    ...
    def act_numpy_vec(self, x):
        ...
        if self.sample:
            ...
            action_indices = np.array(
                [np.random.choice(np.arange(self.du), p=p) for p in probs]
            )
        else:
...
class LinUCB(Controller):
    ...
    def act(self, x):
        ...
        if len(self.batch["context_rewards"][0]) < 1:
            hot_vector = np.zeros(self.env.act_num)
            index = np.random.choice(self.env.act_num, size=1, replace=False)
            hot_vector[index] = 1
            self.t += 1
            return hot_vector
        else:
            ...
            if self.const == 0:
                ...
                for i in range(self.env.act_num):
                    value = np.sum(
                        theta.reshape(-1) * self.env.action_set[i, :]
                    ) + self.const * np.sqrt(
                        self.env.action_set[i : (i + 1), :]
                        @ cov_inv
                        @ self.env.action_set[i : (i + 1), :].T
                    )
                    if value > best_value:
                        best_value = value
                        best_arm = i
