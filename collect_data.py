...
def rollin_bandit(env, controller, orig=False):
    ...
    if controller in ["Transformer", "UCB", "LCB"]:
        ...
        controller = OffPolicy(
            env, prob=np.ones(env.act_num) / env.act_num
    ...
    context_next_states = (
        context_next_states.cpu().detach().numpy().reshape(horizon, -1)
    )
        ...
        for _ in tqdm(range(n_envs)):
            ...
            if var == 0.05 and type == "bernoulli":  ##for mix A_0
                ...
                if _ % 2 == 0:
                    ...
                    traj = generate_bandit_single_traj(
                        env, "Thompson", n_hists, n_samples
                    )
                else:
                    raise
...
if __name__ == "__main__":
    ...
    for iii in range(rep_num):
        ...
        if not os.path.exists("datasets"):  ## save result to username
